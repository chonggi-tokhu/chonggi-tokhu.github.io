...
colourgreyPGN.prototype = {
  gameheader1(pgnstring) {
    ...
    for (var i02k9 = 0; i02k9 < pgnstring2.length; i02k9++) {
  ...
  gamehistory2(pgnstring12) {
    var abcsss = this.gamehistory1(pgnstring12);
    for (var i8sss = 0; i8sss < abcsss.length; i8sss++) {
      if (abcsss == undefined) {
        continue;
      } else if (i8sss == abcsss.length - 1) {
        return abcsss;
      }
    }
  },
  rtparsedpgn(pgnstring5) {
    return {
      ...
      gamehistorywr: this.gamehistory1(pgnstring5),
    };
  },
};
